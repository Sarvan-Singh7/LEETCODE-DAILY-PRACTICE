///INTERVAL DP--------------------------------
class Solution {
    public static int f(int nums[], int si, int ei, int dp[][]){
        if(si == ei)return nums[si];   ///return nums[[si]] as it will also contribute
        if(dp[si][ei] != -1)return dp[si][ei];
        int left = nums[si] - f(nums, si+1, ei, dp); ////MINUS KIYA HAI FUNCTION KO AS PERSON1 NE GAIN KIYA NUMS[SI] KO SO - F() MEANS THAT PERSON2 TRIES TO DECREMENT THAT VALUE

        int right = nums[ei] - f(nums, si, ei-1, dp);   ///HERE ALSO MINUS AS PLAYER2 ALSO DECREMENT HERE
    ...
    public boolean predictTheWinner(int[] nums) {
        int n = nums.length;
        int dp[][] = new int[n][n];
        for(int i=0; i<n;i++)Arrays.fill(dp[i], -1);///-1 fill
        return f(nums, 0, n-1, dp) >=0 ?true:false;
