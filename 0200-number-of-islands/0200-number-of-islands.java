...
class Solution { ///AGAR IS Q MEIN DIAGONAL BHI CHECK KARNA HOTA SO ALL DIRECTION KI DFS CHALTI AS SEE 4 KI CHALAI HAI ABHI BAS
    public static void bfs(char[][] grid, int row, int col){
        ...
        while(!q.isEmpty()){
            int [] current = q.poll();     //current indices nikalne ke liye so that uske as pass ke agar land hai toh unko water bana de
            int currRow = current[0];
            int currCol = current[1];
            for(int[] dir : directions){ //har 4 direction main jao
