// class Solution {    // memoization + Recursion Code
...
class Solution {     ///Tabulation Code
    public int minCost(int n, int[] cuts) {
        ...
        for(int c : cuts)arr.add(c);
        arr.add(n);   ///as length n given hai jahan tak ja sakte hain
        Collections.sort(arr);    //sort karna jaruri so that independent portion ban sake

        int dp[][] = new int[arr.size()][arr.size()];
        int len = arr.size();
        for(int i= len -2; i>=1;i-- ){
            for(int j = i; j<=len-2; j++){///start from i   and <= len -2
                int mini = Integer.MAX_VALUE;
                for(int k=i; k<=j; k++){
